...
const MonthlyFuelCostChart: React.FC = () => {
  const getUserId = async () => {
    ...
    if (!user) {
      throw new Error("Vous devez être connecté pour voir les coûts de carburant.");
    }
    return user.id;
  };

  const { data, isLoading, error } = useQuery<MonthlyFuelData[], Error>({
    queryKey: ['monthlyFuelCosts'],
    queryFn: async () => {
