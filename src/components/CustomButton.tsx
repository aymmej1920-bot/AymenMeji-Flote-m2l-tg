...
const buttonVariants = cva(
  "inline-flex items-center justify-center whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
        default: "primary-button-gradient text-primary-foreground shadow-md hover:shadow-lg active:shadow-sm transition-all duration-200", // Apply gradient and shadow
        destructive:
      ...
      size: {
        default: "h-10 px-4 py-2", // Removed rounded-md, now uses rounded-lg from base
        sm: "h-9 rounded-sm px-3",
        lg: "h-11 rounded-lg px-8",
        icon: "h-10 w-10", // Removed rounded-md, now uses rounded-lg from base
      },
    },
    defaultVariants: {
      variant: "default",
