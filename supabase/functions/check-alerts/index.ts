...
serve(async (req) => {
  ...
  try {
    // Create a Supabase client with the service role key for background tasks
    ...
    if (vehicleError) {
      ...
      for (const vehicle of vehicles) {
        // Check if a similar unread notification already exists for this vehicle and maintenance
    ...
    if (documentError) {
      ...
      for (const doc of documents) {
        // Check if a similar unread notification already exists for this document
        const { data: existingNotification } = await supabaseClient
          .from('notifications')
          .select('id')
